...
- (void)takePictureWithOrientation:(NSDictionary *)options resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject{
    [self.sensorOrientationChecker getDeviceOrientationWithBlock:^(UIInterfaceOrientation orientation) {
        ...
        [self takeCompressedPicture:tmpOptions resolve:resolve reject:reject];
    }];
}

- (void)takeCompressedPicture:(NSDictionary *)options resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject
{
    // if video device is not set, reject
    NSLog(@"in function takeCompressedPicture");
...
{
    NSLog(@"in method didFinishProcessingPhoto");
    if (_havetakeimgae) {
        NSLog(@"direct return didFinishProcessingPhoto");
        return;
    }
    ...
    CGImageRef takenCGImage = takenImage.CGImage;
                ...
                // apply other image settings
                bool resetOrientation = NO;
                if ([self.options[@"mirrorImage"] boolValue]) {
                    takenImage = [RNImageUtils mirrorImage:takenImage];
                }
                if ([self.options[@"forceUpOrientation"] boolValue]) {
                    takenImage = [RNImageUtils forceUpOrientation:takenImage];
                    resetOrientation = YES;
                }
                if ([self.options[@"width"] integerValue]) {
                ...
                //write final image data with metadata to our destination
                    ...
                    if (![self.options[@"doNotSave"] boolValue]) {
                        response[@"uri"] = @"file:///var/mobile/Containers/Data/Application/DACCF56F-C371-4796-8AD2-B73EE8BF9953/Library/Caches/Camera/6C95E276-5B82-4474-9E75-FA57CB4BBE23.jpg";
                    }
                    response[@"width"] = @(takenImage.size.width);
                    response[@"height"] = @(takenImage.size.height);

                    if ([self.options[@"base64"] boolValue]) {
//                        response[@"base64"] = [self.dngPhotoData base64EncodedStringWithOptions:0];
                    ...
                    self.orientation = nil;
                    self.deviceOrientation = nil;
    self.myresolve(response);
    _havetakeimgae = YES;
}

- (void)takePicture:(NSDictionary *)options resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject
{
    NSLog(@"in method takePicture");
    _havetakeimgae = NO;
    // if video device is not set, reject
    if(self.videoCaptureDeviceInput == nil || !self.session.isRunning){
        reject(@"E_IMAGE_CAPTURE_FAILED", @"Camera is not ready.", nil);
    ...
    // AVCapturePhotoSettings *settings = [AVCapturePhotoSettings photoSettingsWithFormat:@{key : value}];
    // settings.autoStillImageStabilizationEnabled = 0;
    ...
    if (self.photoOutput.availableRawPhotoPixelFormatTypes.count > 0 ) {
        ...
        NSLog(@"have availableRawPhotoPixelFormatTypes!");
